<?php
/**
 * Buzzjuice Socials -> BuddyBoss synchronization adapter.
 * Include once from QuickDate's loaded core/bootstrap context.
 */
defined('BZJ_SOCIALS_SYNC_LOADED') || define('BZJ_SOCIALS_SYNC_LOADED', true);

if (!function_exists('bzj_socials_sync_log')) {
    function bzj_socials_sync_log($message, $context = array()) {
        $dir = dirname(__DIR__, 2) . '/data/logs';
        if (!is_dir($dir)) @mkdir($dir, 0755, true);
        $entry = array('time'=>gmdate('c'),'message'=>(string)$message,'context'=>$context);
        @file_put_contents($dir.'/bzj-socials-sync.log', json_encode($entry, JSON_UNESCAPED_SLASHES).PHP_EOL, FILE_APPEND|LOCK_EX);
    }
}
if (!function_exists('bzj_socials_sync_command')) {
    function bzj_socials_sync_command($operation, $actor_id, $target_id) {
        ...
        $body=json_encode(array('origin'=>'socials','operation'=>(string)$operation,'actor_id'=>$actor,'target_id'=>$target,'event_id'=>$event,'authenticated_id'=>$actor),JSON_UNESCAPED_SLASHES);
        ...
        if($errno||$code<200||$code>=300){bzj_socials_sync_log('WordPress synchronization request failed',array('operation'=>$operation,'actor'=>$actor,'target'=>$target,'http_code'=>$code,'curl_error'=>$error));return false;}
        $data=json_decode((string)$raw,true);if(!is_array($data)||empty($data['success'])){bzj_socials_sync_log('WordPress synchronization rejected',array('operation'=>$operation,'actor'=>$actor,'target'=>$target,'response'=>$raw));return false;}
        return true;
    }
}
